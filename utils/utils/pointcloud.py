...
### 
def random_sample_rotation2(rotation_factor: float = 1.0) -> np.ndarray:
    # angle_z, angle_y, angle_x
    eulerz = np.random.rand(3) * np.pi * 2 / rotation_factor  # (0, 2 * pi / rotation_range)
    eulery = np.random.rand(3) * np.pi/6  -np.pi/12 # (-pi/12,  pi/12 )
    eulerx = np.random.rand(3) * np.pi/6  -np.pi/12 # (-pi/12,  pi/12)
    euler2 = np.zeros(3)
    euler2[0] = eulerz[0]
    euler2[1] = eulery[0]
    euler2[2] = eulerx[0]

    rotation = Rotation.from_euler('zyx', euler2).as_matrix()
    return rotation

def random_sample_yaw(rotation_factor: float = 1.0) -> np.ndarray:
    # angle_z, angle_y, angle_x
    euler = np.random.rand(3) * np.pi * 2 / rotation_factor  # (0, 2 * pi / rotation_range)
    euler2 = np.zeros(3)
    euler2[0] = euler[0]
