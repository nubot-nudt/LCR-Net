...
from experiments.lcrnet.modules.ops import grid_subsample, radius_search
from utils.utils.torch import build_dataloader
...
def train_loop_detection_collate_fn_stack_mode(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    # handle special keys: [ref_feats, src_feats] -> feats, [ref_points, src_points] -> points, lengths
    feats = torch.cat(collated_dict.pop('pos_feats') + collated_dict.pop('anc_feats') + collated_dict.pop('neg_feats'), dim=0)
    lengths = torch.cat(collated_dict.pop('pos_lengths') + collated_dict.pop('anc_lengths') + collated_dict.pop('neg_lengths'), dim=0)
...
def train_loop_detection_collate_fn_stack_mode_online(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    lengths = torch.cat(collated_dict.pop('pos_lengths') + collated_dict.pop('anc_lengths') + collated_dict.pop('neg_lengths'), dim=0)
    if lengths[2*batch_size]==0:
        lengths=lengths[:2*batch_size]
    feats = torch.cat(collated_dict.pop('pos_feats') + collated_dict.pop('anc_feats') + collated_dict.pop('neg_feats'), dim=0)
    points_list = collated_dict.pop('pos_points') + collated_dict.pop('anc_points') + collated_dict.pop('neg_points')
    points = torch.cat(points_list, dim=0)
...
def train_loop_detection_collate_fn_stack_mode_halfonline(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    lengths = torch.cat(collated_dict.pop('anc_lengths'), dim=0)
    feats = torch.cat(collated_dict.pop('anc_feats'), dim=0)
    points_list = collated_dict.pop('anc_points')
    points = torch.cat(points_list, dim=0)
    

    lengths_c = torch.cat(collated_dict.pop('pos_lengths') + collated_dict.pop('neg_lengths'), dim=0)
    feats_c = torch.cat(collated_dict.pop('pos_feats')  + collated_dict.pop('neg_feats'), dim=0)
    ...
    if precompute_data:
        ...
        collated_dict['lengths'] = lengths
    collated_dict['batch_size'] = batch_size
    collated_dict['lengths_c'] = lengths_c
    collated_dict['feats_c'] = feats_c

    return collated_dict

def all_collate_fn_stack_mode(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    # handle special keys: [ref_feats, src_feats] -> feats, [ref_points, src_points] -> points, lengths
    feats = collated_dict.pop('anc_feats')
    points_list = collated_dict.pop('anc_points')
    lengths = torch.LongTensor([points.shape[0] for points in points_list])
    points = torch.cat(points_list, dim=0)
    ...
    collated_dict['features'] = feats[0]
    if precompute_data:
        input_dict = precompute_data_stack_mode(points, lengths, num_stages, voxel_size, search_radius, neighbor_limits)
        collated_dict.update(input_dict)
    else:
...
def test_loop_detection_collate_fn_stack_mode_online(
    data_dicts, num_stages, voxel_size, search_radius, neighbor_limits, precompute_data=True
):
    r"""Collate function for registration in stack mode.
    ...
    points_list = collated_dict.pop('anc_points')
    lengths = torch.LongTensor([points.shape[0] for points in points_list])
    # lengths = collated_dict.pop('anc_lengths')
    points = torch.cat(points_list, dim=0)
