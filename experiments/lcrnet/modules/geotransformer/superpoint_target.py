...
class SuperPointTargetGenerator_distance(nn.Module):
    def __init__(self, num_targets, distance_threshold):
        super(SuperPointTargetGenerator_distance, self).__init__()
        self.num_targets = num_targets
        self.distance_threshold = distance_threshold

    @torch.no_grad()
    def forward(self, gt_corr_indices, gt_corr_distance):
        r"""Generate ground truth superpoint (patch) correspondences.
        ...
        Returns:
            ...
            gt_corr_overlaps (LongTensor): overlaps of the selected superpoint correspondences.
        """
        gt_corr_masks = torch.lt(gt_corr_distance, self.distance_threshold)
        gt_corr_distance = gt_corr_distance[gt_corr_masks]
        gt_corr_indices = gt_corr_indices[gt_corr_masks]
        ...
        if gt_corr_indices.shape[0] > self.num_targets:
            ...
            sel_indices = torch.from_numpy(sel_indices).cuda()
            gt_corr_indices = gt_corr_indices[sel_indices]
            gt_corr_distance = gt_corr_distance[sel_indices]

        gt_ref_corr_indices = gt_corr_indices[:, 0]
        gt_src_corr_indices = gt_corr_indices[:, 1]

        return gt_ref_corr_indices, gt_src_corr_indices, gt_corr_distance