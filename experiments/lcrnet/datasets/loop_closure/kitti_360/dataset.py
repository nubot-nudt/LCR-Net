...
def make_dataset_kitti(txt_path, mode, seq=None):
        ...
        #     datasets = []
        #     for seq in seq_list:
        #         datasets += [load_gt_pair_distance_loop(txt_path, seq)]
        #     datasets = np.concatenate(datasets)
        # else:
        datasets = []
        for seq in seq_list:
            datasets += (load_gt_pair_registration(txt_path, seq))
           
        return datasets

class OdometryKitti360Dataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        super(OdometryKitti360Dataset, self).__init__()
        ...
        self.metadata = make_dataset_kitti(self.dataset_360_root+lc_text_root, subset, seq)
