...
from experiments.lcrnet.datasets.utils.mulran import load_mulran_gt_pair_registration
...
def make_dataset_kitti(txt_path, mode):
        ...
        dataset = []
        for seq in seq_list:
            dataset += (load_mulran_gt_pair_registration(txt_path, seq))
...
class OdometrymulranPairDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.metadata = make_dataset_kitti(self.mulran_root+reg_text_root, subset)
    ...
    def __getitem__(self, index):
        ...
        ref_points = self._load_point_cloud(osp.join(self.mulran_root, 'downsampled_xyzi', data_dict['seq_id'], '%d.npy' % (data_dict['pos_idx'])))[:,:3]
        src_points = self._load_point_cloud(osp.join(self.mulran_root, 'downsampled_xyzi', data_dict['seq_id'], '%d.npy' % (data_dict['anc_idx'])))[:,:3]
