...
class LCRNet(nn.Module):
    ...
    def GlobalDescritionHEAD(self, feats_c):
    ...
    def KeypointDetection(self, data_dict):
        ...
        # #########################################
        # # 2. transformer
        enhanced_pos_feats_c, enhanced_anc_feats_c= self.transformer(
        ...
        # 2.1 overlap score
        enhanced_feats_c = torch.cat([enhanced_pos_feats_c.squeeze(0), enhanced_anc_feats_c.squeeze(0)], dim=0)
        feats_list[-1] = enhanced_feats_c

        vote_dict = self.vote_encoder(enhanced_feats_c, data_dict)

        return vote_dict, feats_list, pos_feats_c, anc_feats_c
    ...
    def forward(self, data_dict):
        ...
        vote_dict, feats_list, pos_feats_c, anc_feats_c = self.KeypointDetection(data_dict)

        pos_feature_global = self.GlobalDescritionHEAD(pos_feats_c)
        anc_feature_global = self.GlobalDescritionHEAD(anc_feats_c)
        output_dict['pos_feature_global'] = pos_feature_global
        output_dict['anc_feature_global'] = anc_feature_global
        ...
        if self.vis:
            vis_shifte_node(output_dict, vote_dict,
                color=[1,0.5,0],
...
# 
            visualization(
                output_dict,
                src_node_color = [0, 1, 0],
                ref_node_color = [1, 0, 0],
                src_point_color = [0.1, 0.6, 0.1],
                ref_point_color = [0.6, 0.1, 0.1],
                offsets=(0, 0, -60),    
            )
